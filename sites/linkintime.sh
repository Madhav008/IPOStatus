...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <clientId> <pan> <keyWord>"
    exit 1
fi
...
response=$(curl --insecure -i 'https://linkintime.co.in/mipo/IPO.aspx/SearchOnPan' \
...
status_code=$(echo "$response" | grep -i -E '^HTTP' | awk '{print $2}')
echo "Status Code: $status_code"

# Capture and print the response body
response_body=$(echo "$response" |  awk '/^HTTP\//{p=0} p; /^$/ {p=1}' )
